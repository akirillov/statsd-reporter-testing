...
/**
  ...
  */
private[statsd] object StatsdMetricType {
  val COUNTER = "c"
  val GAUGE = "g"
  val TIMER = "ms"
  val Set = "s"
...
private[statsd] class StatsdReporter(
  ...
  private def send(name: String, value: String, metricType: String)
                  (implicit socket: DatagramSocket): Unit = {
    val tagsPart = if(tags.nonEmpty) s",${tags.mkString(",")}" else ""
    val metricString = s"$name$tagsPart:$value|$metricType"

    logDebug(s"[StatsdReporter] Sending: $metricString", null)

    val bytes = sanitize(metricString).getBytes(UTF_8)
